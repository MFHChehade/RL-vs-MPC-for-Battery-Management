...
class Agent:
    def __init__(self, env_id, algorithm, num_envs=8, log_dir=None):
        ...
        self.vec_env = self._create_vectorized_env(log_dir=self.log_dir) # Create the vectorized environment
        self.model = self._create_model() # Create the model

    ## ------------------- Initialization ------------------- ##
    
    def _create_vectorized_env(self, log_dir = None):
        ...
        return VecMonitor(vec_env, log_dir)
    ...
    def train(self, total_timesteps=5e6, callback_freq=100, num_runs=1):
        self.log_dir_runs = [f"{self.log_dir}/run_{run}" for run in range(num_runs)]
        for run in range(num_runs):
            run_log_dir = f"{self.log_dir}/run_{run}"
            os.makedirs(run_log_dir, exist_ok=True)
            self.vec_env = self._create_vectorized_env(log_dir=run_log_dir)
            self.model = self._create_model()
            callback = SaveOnBestTrainingRewardCallback(check_freq=callback_freq, log_dir=run_log_dir)
            self.model.learn(total_timesteps=total_timesteps, callback=callback)
            self.model.save(f"{self.algorithm}_energy_management_model_run_{run}")
            print(f"Training Done for run {run}")
    ...
    def plot_training_curve(self): # Plot the training curve
        plot_results(self.log_dir_runs, title=f"{self.algorithm} Training Curve")
