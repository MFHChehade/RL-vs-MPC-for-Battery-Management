...
class EnergyManagementEnv(gym.Env):
    def __init__(
        ...
        # Define a discrete action space with 3 possible actions: -0.1, 0, 0.1
    ...
    def load_data(self, data_path):
        ...
        # If date is not datetime, do convert it to datetime
        # check first if there is a date column
        ...
        # If column is called Tariff, rename it to Price
        if 'Tariff' in data.columns:
            data.rename(columns={'Tariff': 'Price'}, inplace=True)
    ...
    def reset_test(self):
        # Set the initial state based on the first time step in the dataframe
        self.state = self.get_initial_state(x = 768 - 23)
        self.bt_old = 0.0  # Reset bt_old
        self.steps = 0
        return self.state
    ...
    def step(self, action):
        ...
        if isinstance(action, np.ndarray) and action.ndim != 0:
            bt = action[0] # continuous action space
        else:
            bt = (action - 1) * 0.1 # discrete action space
        ...
        if self.current_index == len(self.data)-23:
            self.current_index = 0
